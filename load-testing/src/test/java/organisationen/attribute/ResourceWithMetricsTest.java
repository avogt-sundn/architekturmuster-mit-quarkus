 ...
 */
@QuarkusTest
@TestHTTPEndpoint(ResourceWithMetrics.class)
class ResourceWithMetricsTest {
    ...
    void SaveObjectFromJson() {
        ...
        final String location = given().with().contentType(ContentType.JSON).body("""
                ...
                """.formatted(randomUUID)).post()
                .then().log().all()
                .statusCode(equalTo(HttpStatus.SC_CREATED))
                .header("Location", matchesPattern(locationHeaderMatchingPattern))
    ...
    void SearchJsonObject() {
        given().with()
                .param("surname", "vogt")
                .when().get("virtual").then().assertThat()
